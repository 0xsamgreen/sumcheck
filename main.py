...
class Prover:
    def __init__(self, g: sp.Poly) -> None:
    ...
    def executeSumCheck(self, prover: Prover) -> None:
        ...
        for round in range(self.rounds + 1):
            ...
            if round > 0:
                ...
                assert sp.degree(g_j, gen=self.variables[round-1]) == \
                    sp.degree(self.g, gen=self.variables[round-1])
            ...
            if round == 0:
                ...
                assert last_g_j.subs(random) == g_j(0) + g_j(1)

            last_g_j = g_j
        
        random = self.getRandom(self.rounds + 1)
        assert last_g_j.subs(random) == self.oracle.subs(random)
